...
describe("test net.http.parse.request", function()
    it('can parse header lines that terminate by CRLF', function()
        ...
        assert.are.equal( #msg, ParseHeader( header, msg ) )
        assert.are.same({
            foo = 'foo-value',
            bar = 'bar-value',
        }, header )
    ...
    it("can parse header lines from specified offset position", function()
        ...
        assert.are.equal( #msg, ParseHeader( header, msg, #line ) )
        assert.are.same({
            foo = 'foo-value',
            bar = 'bar-value',
        }, header )
    ...
    it("can limit the number of header line", function()
        ...
        assert.are.equal( #msg, ParseHeader( header, msg, nil, nil, 4 ) )
        assert.are.same({
            foo = 'foo-value',
            bar = 'bar-value',
            baz = 'baz-value',
    ...
    it("cannot parse invalide header value", function()
        ...
        for i = 0x0, 0x7e do
            if not VCHAR[i + 1] then
                ...
                assert.are.equal(
                    Parse.EHDRVAL,
                    ParseHeader( {}, 'Host: ' .. c .. 'example.com\n\n' )
                )
            end
        end
    end)
